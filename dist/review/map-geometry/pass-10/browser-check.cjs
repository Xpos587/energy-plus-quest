...
const artifacts = '/home/michael/.local/state/energy-plus-quest/production/2026-09-16-geometry-reference/pass-10';
const url = 'http://localhost:4173/review/map-geometry/pass-10/index.html';
...
(async () => {
  ...
  const results = [];
  for (const [width,height] of [[375,667],[1440,900],[320,568],[390,844]]) {
  ...
  await browser.close();
  const output = {passed:true,results,unchangedMainIndex:true,unchangedPass09Files:true,unchangedCharacterReferences:true,scope:'Static map and stationary cab-side smoke only; no speed or route claim.'};
