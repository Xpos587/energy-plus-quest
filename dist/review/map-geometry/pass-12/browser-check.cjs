...
const artifacts='/home/michael/.local/state/energy-plus-quest/production/2026-09-16-geometry-reference/pass-12';
const url='http://localhost:4173/review/map-geometry/pass-12/index.html';
...
(async()=>{
 ...
 try {
  for(const [width,height] of [[1440,1000],[390,844],[320,568],[820,1180],[568,320]]){
   ...
   assert.equal(await page.locator('[data-variant]').count(),4);
   ...
   for(const key of ['b','c','control','a']){
    await page.locator(`[data-variant="${key}"]`).click();
    assert.equal(await page.locator(`[data-variant="${key}"]`).getAttribute('aria-pressed'),'true');
    assert.equal(await page.locator('#art').getAttribute('href'),key==='control'?'../pass-11/map.jpg':`map-${key}.jpg`);
   ...
   assert.deepEqual(errors,[]);results.push({width,height,passed:true,switches:4,zoomStable:true,reducedMotion:true});
   await page.close();
  }
